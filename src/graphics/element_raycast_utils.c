...
static void	get_sprite_transform(t_game *game, t_element *e, t_element_raycast *r)
{
	float rel_x = e->x - game->player_pos.x;
	float rel_y = e->y - game->player_pos.y;
	float inv_det = 1.0f / (game->camera_plane.x * game->player_dir.y
	                      - game->player_dir.x * game->camera_plane.y);
	r->transform_x = inv_det * (game->player_dir.y * rel_x - game->player_dir.x * rel_y);
	r->transform_y = inv_det * (-game->camera_plane.y * rel_x + game->camera_plane.x * rel_y);
}
...
void compute_sprite_data(t_game *game, t_element *e, t_element_raycast *d)
{
	...
	d->screen_x = (int)((WINDOW_WIDTH / 2) * (1 + d->transform_x / d->transform_y));
