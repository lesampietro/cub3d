...
static void hit_point(t_game *game, t_wall *wall)
{
	if (game->raycasting->hit_side == VERTICAL)
		wall->point = game->player_pos.y + game->raycasting->perp_wall_dist * game->raycasting->ray_dir.y;
	else
		wall->point = game->player_pos.x + game->raycasting->perp_wall_dist * game->raycasting->ray_dir.x;
...
static void get_texture_point(t_game *game, t_wall *wall)
{
	...
	if ((game->raycasting->hit_side == VERTICAL && game->raycasting->direction.x < 0) ||
...
void render(t_game *game, t_wall *wall, int pixel)
{
	...
	while (y < wall->line_end)
	{
		...
		color = get_texture_pixel(game->texture_buf, wall->texture.x, texture_y_pos);
...
void draw_wall(t_game *game, t_raycasting *ray, int pixel)
{
	...
	wall.texture_pos = (wall.line_start - WINDOW_HEIGHT / 2 + wall.height / 2) * wall.resize;
