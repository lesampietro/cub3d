...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/10 23:54:10 by fcaldas-          #+#    #+#             */
/*   Updated: 2025/05/10 23:54:11 by fcaldas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	side_distance(t_game *game, t_raycasting *ray)
{
	...
	if (ray->ray_dir.x < 0)
	{
		ray->side_dist.x = (game->player_pos.x - ray->map_pos.x) \
			* ray->delta_dist.x;
	}
	else
	{
		ray->side_dist.x = (ray->map_pos.x + 1.0 - game->player_pos.x) \
			* ray->delta_dist.x;
	}
	if (ray->ray_dir.y < 0)
	{
		ray->side_dist.y = (game->player_pos.y - ray->map_pos.y) \
			* ray->delta_dist.y;
	}
	else
	{
		ray->side_dist.y = (ray->map_pos.y + 1.0 - game->player_pos.y) \
			* ray->delta_dist.y;
	}
}
...
void	draw_raycasting(t_game *game)
{
	...
	while (pix < WINDOW_WIDTH)
	{
		...
		ray->camera_pixel = multiply_vector(game->camera_plane, \
			ray->plane_multiply);
